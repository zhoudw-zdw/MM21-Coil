...
from utils.data import iCIFAR10, iCIFAR100, iImageNet1000,iImageNet100


class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment,longtail=0):
        ...
        print('training class num  order', self._increments)
        offset = len(self._class_order) - sum(self._increments)
        if offset > 0:
            self._increments.append(offset)
        self.longtail=longtail
        if 'cifar' in dataset_name:
            self.longtaillist=get_img_num_per_cls(500,100,'exp',0.1)
        else:
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        ...
        if shuffle==1:
            print('Shuffle is True, shuffle dataset.')   
            np.random.seed(seed)
